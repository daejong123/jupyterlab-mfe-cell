...
class ChoiceComponent extends React.Component<IProps, any> {
    ...
    private handleMfeObj(key: string, value: any) {
        ...
        mfeObj[key] = value;
        this.props.onChangeContent && this.props.onChangeContent(title, mfeObj, index);
    }
    ...
    shouldComponentUpdate(nextProps: IProps, nextState: any) {
        ...
        const { title: nextTitle, option: nextOption, answer: nextAnswer, explain: nextExplain, canEdit: nextCanEdit } = nextProps;
    ...
    render() {
        const { canEdit = false, type, title, option, answer, explain } = this.props;
        const { userSelectAnswer } = this.state;
        const radioStyle = {
            display: 'block',
            height: '30px',
                                    ...
                                    <Radio style={radioStyle} value={choiceOption} key={index}>
                                        {choiceOption}.
                                        {
                                            canEdit === true ?
                                                (
                                                    <React.Fragment>
                                                        <Input defaultValue={choiceContent} onBlur={(e) => this.onOptionChange(e, choiceOption)} />
                                                        {type === ChoiceType.choice && (
                                                            <React.Fragment>
                                                                {(option as string[]).length > 2 && <Button size="small" type="danger" style={marginLeft} onClick={(e) => this.onDeleteChoiceOption(e, index)}>删除</Button>}
                                                                <Button size="small" type="primary" style={marginLeft} onClick={(e) => this.onAddChoiceOption(e, index, AddDirection.up)}>上面添加</Button>
                                                                <Button size="small" style={marginLeft} onClick={(e) => this.onAddChoiceOption(e, index, AddDirection.down)}>下面添加</Button>
                                                            </React.Fragment>
                                                        )
                                                        }
                                                    </React.Fragment>
